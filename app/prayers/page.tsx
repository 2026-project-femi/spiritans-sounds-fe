...
export default async function PrayersPage({ searchParams }: { searchParams: Promise<{ [key: string]: string | string[] | undefined }> }) {
	...
	const [prayers, totalPrayers] = await Promise.all([
		client.fetch(PRAYERS_QUERY, { start: startIndex, end: endIndex }),
		client.fetch(PRAYERS_COUNT_QUERY),
	]);

	const totalPages = Math.ceil(totalPrayers / PRAYERS_PER_PAGE);
