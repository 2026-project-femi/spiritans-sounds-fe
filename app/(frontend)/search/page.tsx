...
export default async function SearchPage({ searchParams }: SearchPageProps) {
	...
	if (query) {
		...
		const [homily, articles, prayers] = await Promise.all([
			...
			payload.find({ collection: 'articles', where: searchWhere, limit: 50 }),
			payload.find({ collection: 'prayers', where: searchWhere, limit: 50 }),
		]);
	...
	const recentArticlesRes = await payload.find({ collection: 'articles', sort: '-publishedAt', limit: 5 });
	...
	const recentPosts = [
		...recentHomiliesRes.docs.map((d: any) => ({ ...d, _id: d.id, type: 'homily', imageUrl: d.featuredImage?.url, publishedAt: d.date })),
		...recentArticlesRes.docs.map((d: any) => ({ ...d, _id: d.id, type: 'article', imageUrl: d.featuredImage?.url }))
